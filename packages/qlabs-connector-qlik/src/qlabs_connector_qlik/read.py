...
4. **``dataset_refs`` / ``glossary_term_refs`` are left empty.** Both are
   ``list[UUID]`` fields on :class:`~qlabs_catalog_sync_sdk.models.DataProduct` in the
   neutral model — engine-assigned ``neutral_id`` values, resolved via the engine's
   IdentityMap, which a single connector's read cannot know. Qlik's raw
   ``datasetIds``/``apiConsumableDatasetIds``/``glossaryIds`` arrays are opaque native
   ids, not UUIDs, and are not fabricated into either field; they travel losslessly in
   ``custom_attributes`` instead (decision D2: never invent resolution, only report).
   Resolving them is the engine's job, not this module's.
...
async def read(http: HttpEndpoint, ref: IdentityRef, *, endpoint: str) -> NeutralEntity:
    ...
    if ref.entity_type is EntityType.DATA_PRODUCT:
        return await read_data_product(http, ref, endpoint=endpoint)
...
async def read_data_product(http: HttpEndpoint, ref: IdentityRef, *, endpoint: str) -> DataProduct:
    ...
    return _map_data_product(raw, headers, endpoint=endpoint, tenant_id=ref.tenant_id)
