...
- the completion tests use `RM-01-one-way-sync-mvp` — replace with the tag+slug of one of your
  own items (a good choice is one of the five you retired in Phase 3, since it has a full ledger);
...
- the waiver test uses a tag that exists but is not being completed (`RM-02` in the source).
