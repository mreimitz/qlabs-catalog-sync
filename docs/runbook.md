...
| --- | --- | --- |
| **Credentials wrong** (bad client id/secret, expired token, revoked service principal/OAuth client) | **3** (endpoint unreachable) | An error report / log line with `"kind": "AuthError"` and the failing `endpoint`/`operation` named. This is what `/healthz` calling that component `degraded` almost always means. Fix: for a console-configured endpoint (secret reference `db:<name>`), re-enter the credential in the console's **Credentials** panel -- it takes effect on the next cycle with no restart. For an endpoint resolving through `env:<PREFIX>`, correct the variable in `.env` / the secret manager and restart. |
