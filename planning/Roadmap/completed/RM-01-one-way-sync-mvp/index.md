...
* [Upstream metadata sync MVP (Databricks to Qlik)](item.md) - Ship the v1 upstream-only metadata sync: read data product metadata from Databricks and write it into Qlik through the neutral model. Qlik is the sole writer; no two-way sync and no access-control sync in v1.
